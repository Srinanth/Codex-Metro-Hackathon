...
function ReviewStep({ profile, error, onEdit, onConfirm }: { profile: BusinessProfile; error: string | null; onEdit: () => void; onConfirm: () => void }) { return <><p className="text-sm text-accent">Review before saving</p><h2 className="mt-1 text-xl font-semibold">Does this look right?</h2><p className="mt-2 text-sm text-muted">BizzAI only saves this profile after you confirm it.</p><dl className="mt-7 space-y-5"><Detail label="Business name"><span>{profile.name}</span></Detail><Detail label="Business type"><span className="capitalize">{profile.businessType}</span></Detail><Detail label="Capabilities"><div className="flex flex-wrap gap-2">{profile.capabilities.map((capability) => <span key={capability} className="rounded-full border border-border bg-[#0F172A] px-2.5 py-1 text-xs text-accent">{capability}</span>)}</div></Detail><Detail label="Configuration"><DataDetails value={profile.configuration} /></Detail><Detail label="Rules">{profile.rules.length ? <ul className="space-y-2 text-sm text-muted">{profile.rules.map((rule) => <li key={rule}>• {rule}</li>)}</ul> : <span className="text-sm text-muted">No rules provided</span>}</Detail></dl>{error && <Notice message={error} />}<div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button variant="outline" size="lg" onClick={onEdit}><ChevronLeft className="mr-1 h-4 w-4" />Edit</Button><Button size="lg" onClick={onConfirm}>Confirm and save<Check className="ml-2 h-4 w-4" /></Button></div></>; }
