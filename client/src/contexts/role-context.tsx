...
export function RoleProvider({ children }: { children: React.ReactNode }) { const [role, setRoleState] = useState<UserRole | null>(null); useEffect(() => { const saved = localStorage.getItem("role"); if (saved === "owner" || saved === "customer") setRoleState(saved); }, []); const setRole = (next: UserRole) => { localStorage.setItem("role", next); setRoleState(next); }; const clearRole = () => { localStorage.removeItem("role"); localStorage.removeItem("ownerBusinessId"); localStorage.removeItem("ownerBusinessName"); setRoleState(null); }; return <RoleContext.Provider value={{ role, setRole, clearRole }}>{children}</RoleContext.Provider>; }
